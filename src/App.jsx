...
function Wrapper({children}){
  return (
    <div className="light-gradient dark:dark-gradient font-[Noto_Sans]">
      <div className="max-w-[1440px] mx-auto px-4.25 pt-5.25 pb-20 min-h-screen ">
        {children}
      </div>
    </div>
  )
...
function ExtensionCard({name, description, logo, isActive, setExtensionsList}){
  ...
  return (
    <div className={` bg-white dark:bg-clr-neutral-700 shadow-lg rounded-3xl grid grid-cols-[auto_1fr] _outline px-4.5 py-4.75 gap-x-4 gap-y-6 desktop:px-4.75 desktop:pt-4.25 grid-rows-[1fr_auto]`}>
      <div className="">
        <img src={logo} alt="extension icon" />
      </div>
      <div>
        <h2 className="font-bold text-[1.3rem]/6 mb-2 text-clr-neutral-900 desktop:text-xl/6">{name}</h2>
        <p className="text-[15px]/5.5 text-clr-neutral-600">{description}</p>
      </div>
      <div className="col-span-2 flex justify-between items-center pt-0.25">
        <button className="border border-black/30 pl-[calc(var(--pad-x)+1px)] pr-[calc(var(--pad-x)-1px)] h-9 rounded-2xl tracking-tight [--pad-x:--spacing(4)]">Remove</button>
...
function FilterBtn({selected, children, setCurrentFilter}){
  ...
  return (
    <button className={`cursor-pointer shadow-lg px-4.75 h-11 rounded-3xl text-xl  ${selected? "bg-clr-red-700 dark:bg-clr-red-500 dark:text-black text-white": "bg-white text-clr-neutral-900 dark:bg-clr-neutral-700 dark:text-white ring ring-clr-neutral-300/40 "}`} onClick={handleClick}>
      {children}
    </button>
  )
}
